...
func (a *App) GetUserByNicknameUA(nickname string) (*model.User, *model.AppError) {
	...
	if err != nil {
		...
		switch {
		case errors.As(err, &nfErr):
			return nil, model.NewAppError("GetUserByNickname", "app.user.get_by_nickname.app_error", nil, nfErr.Error(), http.StatusNotFound)
		default:
			return nil, model.NewAppError("GetUserByNickname", "app.user.get_by_nickname.app_error", nil, err.Error(), http.StatusInternalServerError)
		}
	}
	return result, nil
}
